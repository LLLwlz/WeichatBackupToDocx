...
def decrypt(key):
    ...
    conn = sqlite.connect("WxFileIndex.db")
    c = conn.cursor()
    ...
    try:
        ...
        c.execute("ATTACH DATABASE 'WxFileIndex-decrypted.db' AS wechatdecrypted KEY '';")
...
def generate_key():
    ...
    key = hashlib.md5(str(str(imei) + str(uin)).encode('utf-8')).hexdigest()[0:7]
